...
Ngoại lệ duy nhất: Level 1–2 của knowledge map (`g15` Parts of Speech, `g16` Phrases) là
bài nền theo lộ trình của nghiên cứu viên — đừng xoá, xem mục 13 ngày 2026-09-15. Câu hỏi
trong hai bài đó vẫn không được dùng cấu trúc ngoài danh mục.
...
| 2026-09-13 | **Dịch lớp học liệu JSON**: 600 câu lệnh Từ vựng (4 khung), 36 tiêu đề, 25 dòng bảng kiểm, 14 tên hạng mục lỗi | Theo phương án nghiên cứu viên chọn. Giữ tiếng Việt: `explanation` và `meaning` (mục 15 miễn trừ); `options` — cả 600 đều là nghĩa từ của 150 câu Word → Meaning, dịch sang Anh là đổi dạng bài; `theory` — phần giảng ngữ pháp. Sửa tay JSON vì không tìm thấy bộ sinh trong repo này: Pipeline sinh lại ở nơi khác sẽ ghi đè, phải sửa theo |
| 2026-09-13 | Khung câu lệnh Từ vựng và 4 tiền tố so khớp trong `practice/vocabulary/js/app.js` đổi **cùng một commit**; `CONTENT_VERSION` lên `c1.0.2` | Nhãn dạng câu hỏi dựa vào tiền tố câu lệnh, lệch là hỏng âm thầm. Kiểm từng câu trong 600 câu: phân loại trước và sau trùng 100% (200 · 150 · 150 · 100). Đổi câu lệnh là đổi item, và mọi dòng log mang `content_version`, nên câu trả lời dưới câu lệnh tiếng Việt vẫn tách riêng được |
| 2026-09-13 | `loader.js`: URL tải kèm `?v=${VER}`, và gọi `purgeOldCache()` khi khởi động | Hai lỗi có sẵn mà lần tăng phiên bản này sẽ làm lộ ra. URL không kèm phiên bản trong khi Pages trả `max-age=600` và khoá localStorage không bao giờ hết hạn, nên máy tải trong 10 phút đầu có thể cất JSON cũ dưới khoá mới vĩnh viễn. Hàm xoá khoá cũ có sẵn nhưng chưa từng được gọi, trái DATA-DESIGN §10 — mỗi lần tăng phiên bản lại để lại một bản sao học liệu, chung quota với hàng đợi dữ liệu |
| 2026-09-13 | **Khoá `build_vocab.py`**; JSON Từ vựng đã commit là nguồn gốc duy nhất | Script chỉ sinh 10 câu/unit, chạy lại rồi commit là xoá 500/600 câu. Bộ sinh ngân hàng 60 câu (`fb65b2e`, `26af531`) không có trên đĩa lẫn trong git. Các script sinh học liệu chỉ nằm ở `C:/hsg2627.github.io/scratch/` — bị gitignore, và ghi thẳng vào `content/` |
| 2026-09-13 | Đồng bộ `build_grammar`, `build_skills_and_exams`, `build_ai_eval` với nội dung đang chạy | Trước đó chạy lại sẽ trả tiêu đề, bảng kiểm, tên hạng mục về tiếng Việt và mất các lời giải thích sửa ở `604af06`. Kiểm bằng chạy bản sao trong thư mục tạm: đầu ra khớp `d64bc14` từng giá trị. Bản gốc ở `scratch/backup/scripts-2026-09-13/`. **Không chạy script sinh vào repo khi chưa chạy thử bản sao và so với `content/`** |
| 2026-09-13 | **Thiết kế lại khu học sinh** theo mẫu tham khảo: dải header tối có cụm trạng thái, bảng điều khiển hai cột ở trang chủ và trang Luyện tập | Header giữ nguyên dòng cảnh báo AI của mục 6 ngay dưới tên dự án. HUD thành các ô tròn trong header. Thẻ lớn là cả một `<a>` (§5.4); bốn mục điều hướng giữ nguyên thứ tự (§5.9) |
| 2026-09-13 | **Không** đưa vào: bảng xếp hạng, ô góp ý tự do, ô ủng hộ, thẻ giới thiệu người biên soạn, chuỗi ngày học | Mã học sinh là lớp + số thứ tự sổ điểm, nên xếp hạng theo mã là lộ danh tính trong lớp — và trang không có máy chủ để so giữa các máy; thay bằng thẻ "Your practice" chỉ đếm của chính em. Ô góp ý là chữ học sinh tự viết, một loại dữ liệu mới; hộp báo lỗi Xưởng AI đã có. Chuỗi ngày học: nghiên cứu viên thấy không cần thiết, đã dựng rồi gỡ trước khi commit |
| 2026-09-13 | Nền **sáng/tối** do người dùng chọn; khoá `dt_theme_v1`; `core/theme-boot.js` nạp trong `<head>` trước CSS | Nạp sau CSS thì người chọn nền tối thấy trang chớp trắng mỗi lần mở. Không phải dữ liệu nghiên cứu: không ghi log, không nằm trong `dt_state_v1`, nút "Xoá dữ liệu học tập" không xoá. Chưa chọn thì theo nền của máy |
| 2026-09-13 | **Lệch §5.1**: thêm token `--band-*`, `--gold`, `--wrap-dash` | Dải header và thẻ lớn tối ở cả hai nền nên không dùng được `--navy` (đảo sang xanh nhạt ở nền tối). Giá trị lấy lại từ cột trái trang đăng nhập; `--gold` là `--brass` của nền tối. Trang làm bài vẫn giữ `--wrap` 680px |
...
| 2026-09-15 | **Trang Luyện tập đi theo knowledge map của nghiên cứu viên**: Ngữ pháp 14 level (+ mục "More grammar"), Từ vựng 4 chapter / 10 level. Thứ tự và tên nằm ở `content/knowledge-map.json`, `id` module giữ nguyên | Map là thứ tự bài nghiên cứu viên tự chia, tên đã chỉnh cho hợp lộ trình học — trùng 14 `grammar_levels` của trang cũ. Tách thứ tự khỏi `id` như `schedules.json` (mục 3): đánh số lại là log cũ trỏ sai bài. Level 14 gồm `g10` + `g11`. Chapter Từ vựng trộn unit HK1 với HK2, nhưng `semester` trong manifest không đổi nên phép lọc HK1/HK2 của RQ3 vẫn nguyên |
| 2026-09-15 | **Ngoại lệ mục 3**: thêm `g15` Parts of Speech (Level 1) và `g16` Phrases (Level 2), `grammar: []`, HK1 | Bài nền của lộ trình, không thêm cấu trúc mới. Lý thuyết và câu hỏi khôi phục từ `grammar_levels` 1–2 của `eng10-units.json`; nghiên cứu viên đưa tài liệu chi tiết cho AI soạn nên mọi câu mang `ai_generated: true`. `g15` giữ đủ 10 câu. `g16`: dịch câu lệnh và phương án sang tiếng Anh (mục 15), bỏ 2 câu dùng cấu trúc ngoài danh mục (cụm phân từ; `has been V-ing`), còn 6 câu |
| 2026-09-15 | `g14` Articles giữ trong Ngữ pháp, ở mục "More grammar" sau Level 14; **gỡ phần lý thuyết** | Map không có Articles; nghiên cứu viên giữ lại và sẽ đưa ngữ liệu bổ sung. Lý thuyết của `g14` từ 27/08 là bài "The 8 Parts of Speech" (`build_grammar.py` gán `source_id: 1`) — học sinh mở tóm tắt Mạo từ lại đọc Từ loại. Để trống đến khi có ngữ liệu |
| 2026-09-15 | `QUEST_POOL` xếp theo thứ tự level, thêm `g15`/`g16` vào HK1; `CONTENT_VERSION` lên `c1.1.0` | Vòng xoay dài ra nên bài của ngày deploy có thể đổi giữa ngày; `ENDPOINT` còn rỗng nên không dòng dữ liệu nào lệch. Không tăng phiên bản thì máy đã cache `manifest.json` cũ sẽ không thấy `g15`/`g16` |
| 2026-09-15 | Trang đọc `knowledge-map.json` bằng `loadJSON`, **không** thêm hàm export mới vào `loader.js` | URL module không gắn phiên bản và Pages cache 10 phút: ngay sau deploy, `app.js` mới có thể gặp `loader.js` cũ, và import một tên loader cũ không có là lỗi liên kết — trắng cả trang (cùng loại lỗi dòng 13/09). Không nạp được map thì trang bài tập giữ tiêu đề cũ, bài vẫn làm được |
| 2026-09-15 | Đồng bộ `build_grammar.py` (clone `C:/`): thêm `g15`/`g16`, `g14` bỏ `source_id`, bỏ câu lý thuyết mặc định | Chạy lại bản cũ sẽ xoá Level 1–2 và gắn lại bài Parts of Speech vào Articles. Ba tệp `g14`–`g16` cài từ chính đầu ra của bản sao chạy trong thư mục tạm; 16/16 tệp khớp `content/grammar/`. Bản trước khi sửa: `scratch/backup/scripts-2026-09-15/` |
| 2026-09-15 | Kiểm giao diện Luyện tập bằng **bản sao không log** ngoài repo: chép `css/`, `content/`, `practice/` sang thư mục tạm, thay `/js/progress.js` bằng stub qua import map | Mục 3 và 11 cấm chạy thử trang vì mỗi cú bấm sinh log. Bản sao không nạp `core/` (có phép kiểm chặn import `core/`): không Spine, không hàng đợi, nên không sinh được dòng nào; trang thật và `localStorage` của miền thật không bị chạm |
